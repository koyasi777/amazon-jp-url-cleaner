...
// @version      2.0.0
// @description  Amazon.co.jp URL cleaner userscript that normalizes links by removing /ref= segments and tracking params for short clean URLs. Product pages become canonical /dp/ASIN, and search or other pages keep only functional params so filters still work. Hooks History/Location APIs, link clicks and SPA navigation to keep URLs readable and more privacy-respecting.
// @description:ja   Amazon.co.jp 向けの URL クリーナーです。/ref= やトラッキング用クエリなど不要なパラメータを自動削除し、短くクリーンな URL に正規化します。商品ページは /dp/ASIN 形式に統一し、検索結果などは必要な検索条件・機能パラメータだけを残します。History/Location API とリンククリック、SPA 遷移をフックして、読みやすく共有しやすいプライバシー配慮の URL を保ちます。
// @description:en   Amazon.co.jp URL cleaner userscript that normalizes links by removing /ref= segments and tracking params for short clean URLs. Product pages become canonical /dp/ASIN, and search or other pages keep only functional params so filters still work. Hooks History/Location APIs, link clicks and SPA navigation to keep URLs readable and more privacy-respecting.
// @description:zh-CN  面向 Amazon.co.jp 的 URL 清理脚本，自动规范化链接，移除 /ref= 片段和各类跟踪查询参数，将网址压缩为简短干净的 URL。商品页统一转换为规范 /dp/ASIN 形式，搜索等页面仅保留必要功能参数以保证筛选和功能正常。通过拦截 History/Location API、链接点击和 SPA 路由，保持网址可读、易分享且更注重隐私。
// @description:zh-TW  面向 Amazon.co.jp 的 URL 清理腳本，自動正規化網址，移除 /ref= 段落與各種追蹤查詢參數，將網址壓縮成簡短乾淨的連結。商品頁會統一轉換為標準 /dp/ASIN 形式，搜尋等頁面只保留必要功能參數以維持篩選與功能正常。透過攔截 History/Location API、連結點擊與 SPA 導覽，保持網址易讀、好分享且更重視隱私。
// @description:ko    Amazon.co.jp 전용 URL 클리너 유저스크립트로 /ref= 구간과 각종 추적용 쿼리 파라미터를 자동 제거해 짧고 깔끔한 링크로 만듭니다. 상품 페이지는 표준 /dp/ASIN 형식으로 재구성하고, 검색 등 다른 페이지에서는 필요한 기능 파라미터만 남겨 필터와 기능이 그대로 동작하도록 합니다. History/Location API, 링크 클릭, SPA 내비게이션을 후킹해 읽기 쉽고 공유하기 편하며 프라이버시를 더 잘 보호하는 URL을 유지합니다。
// @description:fr    Userscript de nettoyage d’URL pour Amazon.co.jp qui normalise les liens en supprimant les segments /ref= et les paramètres de suivi pour obtenir des URL courtes et propres. Les pages produit sont reconstruites au format canonique /dp/ASIN, tandis que les pages de recherche et de navigation ne gardent que les paramètres fonctionnels. Intercepte les API History/Location, les clics sur les liens et la navigation SPA pour conserver des URL lisibles, faciles à partager et plus respectueuses de la confidentialité.
// @description:es    Userscript limpiador de URL para Amazon.co.jp que normaliza enlaces eliminando segmentos /ref= y parámetros de seguimiento para crear URL cortas y limpias. En páginas de producto reconstruye la URL en el formato canónico /dp/ASIN y en páginas de búsqueda u otras solo mantiene los parámetros funcionales para que los filtros sigan funcionando. Engancha las API de History/Location, los clics en enlaces y la navegación SPA para mantener URL legibles, fáciles de compartir y más respetuosas con la privacidad.
// @description:de    Amazon.co.jp-URL-Cleaner-Userscript, das Links automatisch normalisiert, indem /ref=-Segmente und Tracking-Parameter entfernt werden, sodass kurze, saubere URLs entstehen. Produktseiten werden in die kanonische Form /dp/ASIN umgebaut, während auf Such- und anderen Seiten nur funktionale Parameter erhalten bleiben. Hakt sich in History-/Location-APIs, Link-Klicks und SPA-Navigation ein, um URLs dauerhaft lesbar, teilfreundlich und datenschutzfreundlicher zu halten.
// @description:pt-BR Userscript limpador de URL para a Amazon.co.jp que normaliza links removendo segmentos /ref= e parâmetros de rastreamento para gerar URLs curtas e limpas. Em páginas de produto, reconstrói a URL no formato canônico /dp/ASIN; em páginas de busca e navegação mantém apenas os parâmetros funcionais. Conecta-se às APIs History/Location, aos cliques em links e à navegação SPA para manter URLs legíveis, fáceis de compartilhar e mais amigáveis à privacidade.
// @description:ru    Пользовательский скрипт-очиститель URL для Amazon.co.jp, который нормализует ссылки, удаляя сегменты /ref= и трекинговые параметры, чтобы получать короткие и чистые URL. Для товарных страниц формирует канонический вид /dp/ASIN, а на страницах поиска и навигации сохраняет только функциональные параметры. Перехватывает History/Location API, клики по ссылкам и SPA-навигацию, поддерживая URL читаемыми, удобными для обмена и более приватными.
// @author       koyasi777
// @license      MIT
