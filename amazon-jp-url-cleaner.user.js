...
// @version      2.0.0
// @description  Amazon.co.jp URL cleaner userscript. Removes /ref= path segments and common tracking parameters to produce shorter, cleaner links. Product pages are normalized to canonical /dp/ASIN URLs; non-product pages remove known tracking params while preserving other parameters so site features (e.g., filters) keep working. Hooks the History/Location APIs, link clicks, and SPA navigation to keep URLs readable and privacy-friendly.
// @description:ja   Amazon.co.jp 向けの URL クリーナー Userscript。/ref= を含むパス片や一般的なトラッキング用パラメータを削除して、短くクリーンな URL にします。商品ページは正規の /dp/ASIN 形式に正規化し、商品以外のページは既知のトラッキング要素だけを除去しつつ他のパラメータは維持することで、フィルタ等の機能が動作するようにします。History/Location API、リンククリック、SPA 遷移をフックして、常に読みやすくプライバシー配慮の URL を保ちます。
// @description:en   Amazon.co.jp URL cleaner userscript. Removes /ref= path segments and common tracking parameters to produce shorter, cleaner links. Product pages are normalized to canonical /dp/ASIN URLs; non-product pages remove known tracking params while preserving other parameters so site features (e.g., filters) keep working. Hooks the History/Location APIs, link clicks, and SPA navigation to keep URLs readable and privacy-friendly.
// @description:zh-CN  面向 Amazon.co.jp 的 URL 清理脚本：移除路径中的 /ref= 片段及常见跟踪参数，生成更短更干净的链接。商品页统一规范为 /dp/ASIN 的标准形式；非商品页仅删除已知跟踪参数，同时保留其它参数以确保筛选等功能正常。拦截 History/Location API、链接点击与 SPA 路由，使网址更易读、更便于分享且更注重隐私。
// @description:zh-TW  面向 Amazon.co.jp 的 URL 清理腳本：移除路徑中的 /ref= 段落與常見追蹤參數，產生更短更乾淨的連結。商品頁統一正規化為 /dp/ASIN 的標準形式；非商品頁僅刪除已知追蹤參數，同時保留其它參數以維持篩選等功能正常。攔截 History/Location API、連結點擊與 SPA 導覽，讓網址更易讀、好分享且更重視隱私。
// @description:ko    Amazon.co.jp 전용 URL 클리너 유저스크립트입니다. 경로의 /ref= 구간과 일반적인 추적 파라미터를 제거해 더 짧고 깔끔한 링크를 만듭니다. 상품 페이지는 표준 /dp/ASIN 형식으로 정리하며, 비상품 페이지는 알려진 추적 파라미터만 제거하고 나머지 파라미터는 유지해 필터 등 사이트 기능이 정상 동작하도록 합니다. History/Location API, 링크 클릭, SPA 내비게이션을 후킹해 URL을 읽기 쉽고 프라이버시 친화적으로 유지합니다.
// @description:fr    Userscript de nettoyage d’URL pour Amazon.co.jp. Supprime les segments /ref= dans le chemin et les paramètres de suivi courants afin d’obtenir des liens plus courts et plus propres. Les pages produit sont normalisées vers la forme canonique /dp/ASIN ; les pages non produit ne retirent que les paramètres de suivi connus tout en conservant les autres paramètres afin que les fonctionnalités du site (p. ex. les filtres) continuent de fonctionner. Intercepte les APIs History/Location, les clics sur les liens et la navigation SPA pour garder des URL lisibles et plus respectueuses de la vie privée.
// @description:es    Userscript limpiador de URL para Amazon.co.jp. Elimina segmentos /ref= en la ruta y parámetros de seguimiento comunes para generar enlaces más cortos y limpios. En las páginas de producto normaliza la URL al formato canónico /dp/ASIN; en las páginas no relacionadas con productos elimina únicamente los parámetros de seguimiento conocidos y conserva el resto para que las funciones del sitio (p. ej., filtros) sigan funcionando. Engancha las APIs de History/Location, los clics en enlaces y la navegación SPA para mantener URLs legibles y más respetuosas con la privacidad.
// @description:de    Amazon.co.jp-URL-Cleaner-Userscript: Entfernt /ref=-Segmente im Pfad und gängige Tracking-Parameter, um kürzere, saubere Links zu erzeugen. Produktseiten werden in die kanonische Form /dp/ASIN normalisiert; auf Nicht-Produktseiten werden nur bekannte Tracking-Parameter entfernt, während andere Parameter erhalten bleiben, damit Website-Funktionen (z. B. Filter) weiterhin funktionieren. Hakt sich in die History- und Location-APIs, Link-Klicks und SPA-Navigation ein, um URLs dauerhaft gut lesbar und datenschutzfreundlicher zu halten.
// @description:pt-BR Userscript limpador de URL para a Amazon.co.jp. Remove segmentos /ref= no caminho e parâmetros de rastreamento comuns para gerar links mais curtos e limpos. Em páginas de produto, normaliza a URL para o formato canônico /dp/ASIN; em páginas que não são de produto, remove apenas parâmetros de rastreamento conhecidos e preserva os demais para que recursos do site (por exemplo, filtros) continuem funcionando. Intercepta as APIs de History/Location, cliques em links e a navegação SPA para manter URLs legíveis e mais amigáveis à privacidade.
// @description:ru    Пользовательский скрипт-очиститель URL для Amazon.co.jp. Удаляет сегменты /ref= в пути и распространённые трекинговые параметры, чтобы получать более короткие и чистые ссылки. Товарные страницы нормализуются до канонического вида /dp/ASIN; на нетоварных страницах удаляются только известные трекинговые параметры, при этом остальные параметры сохраняются, чтобы функции сайта (например, фильтры) продолжали работать. Перехватывает API History/Location, клики по ссылкам и SPA-навигацию, поддерживая URL читаемыми и более приватными.
// @author       koyasi777
// @license      MIT
